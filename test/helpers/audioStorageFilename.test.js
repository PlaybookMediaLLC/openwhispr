...
const { resolveReleaseDistribution } = require("../../src/helpers/releaseIdentity.js");

installElectronStub();
const AudioStorageManager = require("../../src/helpers/audioStorage.js");
const executableName = resolveReleaseDistribution(
...
test("names saved audio in local time whether or not the timestamp carries a zone", (t) => {
  ...
  assert.equal(
    filenameFor("2026-09-10 12:00:00.000Z"),
    `${executableName}-2026-09-10-05-00-00-42.webm`
  );
  assert.equal(
    filenameFor("2026-09-10 12:00:00"),
    `${executableName}-2026-09-10-05-00-00-42.webm`
  );
});

test("falls back to the bare name when the timestamp is unusable", () => {
  assert.equal(filenameFor(null), `${executableName}-42.webm`);
  assert.equal(filenameFor("not a date"), `${executableName}-42.webm`);
});
