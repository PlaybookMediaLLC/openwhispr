...
class HyprlandShortcutManager {
  ...
  _removeLegacyArtifacts(config) {
    ...
    const removedBindsFiles = new Set();
    const legacyBindsUpdates = [];

    for (const candidate of this._legacyBindsFiles(configDir, legacyBindsPath)) {
      try {
        const content = fs.readFileSync(candidate, "utf-8");
        if (!isManagedBindsContent(content)) continue;
        ...
        if (!hasManagedArtifacts) continue;
        ...
        if (remainingLines.join("\n").trim()) {
          legacyBindsUpdates.push({ candidate, content: remainingLines.join("\n") });
        } else {
          removedBindsFiles.add(path.basename(candidate));
        }
      } catch (err) {
        if (err.code !== "ENOENT") throw err;
      }
      ...
      try {
        ...
        const newContent = content
          ...
          .filter((line) => {
            ...
            return !removedBindsFiles.has(sourced);
          })
          .join("\n");
    ...
    for (const { candidate, content } of legacyBindsUpdates) {
      fs.writeFileSync(candidate, content, "utf-8");
    }
    for (const filename of removedBindsFiles) {
      fs.unlinkSync(path.join(configDir, filename));
    }
  }
